...
\section{JavaScript}
\begin{frame}
 \frametitle{O que é JavaScript?}
\begin{block}{Definição}
    JavaScript é uma \textbf{linguagem de programação} que permite a você implementar itens complexos em páginas web — \textit{toda vez que uma página da web faz mais do que simplesmente mostrar a você informação estática} — mostrando conteúdo que se atualiza em um intervalo de tempo, mapas interativos ou gráficos 2D/3D animados, etc. \\
    Nessas situações o JavaScript provavelmente está envolvido. \\
    É a terceira camada das tecnologias padrões da web, duas das quais HTML e CSS.\\
    \tiny{Fonte: \cite{javascript}}
\end{block}
\end{frame}
